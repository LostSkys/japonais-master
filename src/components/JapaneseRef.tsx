...
const JapaneseRef: React.FC = () => {
  ...
  return (
    <div className="bg-slate-800 rounded-3xl p-4 shadow-2xl animate-in fade-in duration-300 border border-slate-700 w-full max-w-md mx-auto">
      {/* Sélecteur d'onglets style Gaming */}
      ...
      <div className="max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar">
        {view === 'basics' ? (
          <div className="grid grid-cols-2 gap-3">
            {basics.map((item, i) => (
              <div key={i} className="bg-slate-900 p-4 rounded-2xl border-b-4 border-blue-900/50 flex flex-col items-center">
                ...
                <span className="text-slate-400 text-xs mt-1">{item.fr}</span>
              </div>
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-2">
            {alphabetAZ.map((item, i) => (
              <div key={i} className="bg-slate-900 p-3 rounded-xl border border-slate-700 flex flex-col items-center relative overflow-hidden group">
                <span className="absolute top-0 left-0 bg-slate-700 text-[8px] px-1 text-slate-300 font-bold">{item.letter}</span>
                <span className="text-2xl font-bold text-white mt-1">{item.jp}</span>
      ...
      <p className="mt-6 text-center text-slate-500 text-[10px] italic">
        {view === 'basics' ? "Les bases pour comprendre les phrases." : "Équivalents phonétiques pour ton prénom !"}
      </p>
    </div>
  );
};
